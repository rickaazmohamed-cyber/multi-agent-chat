...
import { 
  ...
  Copy, Check, Activity, Bell, ShieldAlert, CheckCheck, Clock
} from 'lucide-react';
...
export default function App() {
  ...
  // === MULTI-AGENT ROSTER ===
  const agents = [
    { id: 'a1', name: 'Sarah', dept: 'Technical Support', color: '#10b981' }, // Emerald
    { id: 'a2', name: 'Marcus', dept: 'Billing & Sales', color: '#f59e0b' },   // Amber
    { id: 'a3', name: 'Chloe', dept: 'Customer Success', color: '#8b5cf6' }    // Violet
  ];
  ...
  useEffect(() => {
    // 1. Point to the 'messages' collection in Firestore
    const messagesRef = collection(db, 'messages');
    
    // 2. Query messages ordered by their creation time
    const q = query(messagesRef, orderBy('createdAt', 'asc'));

    // 3. Listen for real-time updates (onSnapshot)
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      showToast("Could not connect to database. Check rules.", "error");
    });
  ...
  // === FIREBASE: SEND CUSTOMER MESSAGE ===
  const handleCustomerSend = async (e) => {
    if (e) e.preventDefault();
    if (!userInput.trim()) return;

    const messageText = userInput.trim();
    setUserInput(''); // Clear input instantly for snappy UI
  ...
  const handleAdminSend = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'messages'), {
        text: messageText,
        sender: 'agent',
        agentDetails: activeAgent, // Save which specific agent replied
        createdAt: serverTimestamp(),
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      });
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 font-sans">
      ...
      {/* ==========================================
         ...
         ========================================== */}
      <div className="w-full lg:w-[45%] h-full flex flex-col border-r border-slate-800 bg-slate-900">
                ...
                <div className="w-[320px] h-[460px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
                  ...
                  <form onSubmit={handleCustomerSend} className="p-3 border-t bg-white flex gap-2">
                    <input 
                      ...
                      className="flex-1 text-xs px-3 py-2 rounded border border-slate-200 focus:outline-none focus:border-blue-500"
                    />
